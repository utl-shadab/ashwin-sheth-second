...
export default function Footer({ footerRef }: FooterProps) {
  ...
  useEffect(() => {
    ...
    if (isHome) {
      // On home page, MasterSequence controls visibility.
      // Reset inline styles that might have been set during navigation.
      footerRef.current.style.opacity = "";
      footerRef.current.style.pointerEvents = "";
      // Ensure GSAP starting state is respected if MasterSequence hasn't run yet
      // (This is redundant if className handles it, but safe)
    } else {
      // On non-home pages, make footer visible immediately
